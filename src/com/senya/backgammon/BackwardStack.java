...
public class BackwardStack extends Stack{
	...
	public BackwardStack(int capacity, float chipSize, Bitmap[] res, boolean highlight) {
		super(capacity);
		int delay = 0;
		for (int i = 0; i < chips.length; i++) {
			chips[i] = new Chip(0, new Point(BoardPositions.getInstance()
					...
					(i < capacity - 1) ? false : true, chipSize, res, delay, highlight);
			delay += 200;
		}
	}
	public BackwardStack(int capacity, float chipSize, Bitmap[] res, int[] positions, mPoint[] points, boolean[] isTop,boolean highlight) {
		super(capacity);
		int delay = 0;
		for (int i = 0; i < chips.length; i++) {
			chips[i] = new Chip(positions[i], new Point(points[i].x,points[i].y),
					isTop[i], chipSize, res, delay, highlight);
			delay += 200;
		}
	}
