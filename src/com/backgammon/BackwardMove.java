...
public class BackwardMove extends Move{
	...
	public BackwardMove(Stack stack, Stack opponent, float chipSize,
		...
		if(!double_mode){
			if (first != Integer.MAX_VALUE
				...
				res.add(new MoveBind(new Point(
						BoardPositions.getInstance().getOtherwiseAt(
			...
			if ((first != Integer.MAX_VALUE && second != Integer.MAX_VALUE)
					...
					&& opponent.chipsAtPos(BoardPositions.getInstance()
							.ConvertPosition(
									stack.getFocusedChipPosition() + first
											+ second)) == 0 && (opponent.chipsAtPos(BoardPositions.getInstance()
													.ConvertPosition(
															stack.getFocusedChipPosition() + first)) == 0 || opponent.chipsAtPos(BoardPositions.getInstance()
																	.ConvertPosition(
																			stack.getFocusedChipPosition() + second)) == 0) && canSetToPos(stack.getFocusedChipPosition() + first + second)) {
				int offset = stack.chipsAtPos(stack.getFocusedChipPosition()
						+ first + second)
						* (int) chipSize;
				...
				res.add(new MoveBind(new Point(BoardPositions.getInstance()
						...
						.getInstance().getOtherwiseAt(
								stack.getFocusedChipPosition() + first
										+ second).y
						+ offset), stack.getFocusedChipPosition() + first
						+ second,CompleteMove.BOTH));
			}
		}else{
			for (int i = 0; i < movesLeft; i++) {
				if (stack.getFocusedChipPosition() + first
								* (i + 1) < BoardPositions.POSITIONS_COUNT
						&& opponent.chipsAtPos(BoardPositions.getInstance()
								.ConvertPosition(
										stack.getFocusedChipPosition()
												+ first * (i + 1))) == 0 && canSetToPos(stack.getFocusedChipPosition() + first * (i + 1))) {
					...
					if (stack.getFocusedChipPosition() + first * (i + 1) >= BoardPositions.POSITIONS_COUNT / 2)
						offset = -offset;
					res.add(new MoveBind(new Point(BoardPositions
							.getInstance().getOtherwiseAt(
									stack.getFocusedChipPosition() + first
				...
				if ((targetPos == BoardPositions.POSITIONS_COUNT)
					...
					res.add(new MoveBind(
							new Point(BoardPositions.getInstance()
									.getOtherwiseAt(targetPos).x,
									BoardPositions.getInstance()
											.getOtherwiseAt(targetPos).y
